                ...
                <div class="p-2 overflow-scroll px-0">
                    <table class="w-full table-fixed">
                        ...
                        <tr>
                            ...
                            <th class="w-2/6 text-center px-5 border-r">Categoría ID</th>
                            <th class="w-1/6 text-center border-r">Estado</th>
                            <th class="w-2/6 text-center">Acciones</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr *ngFor="let producto of productos; let i = index" class="hover:bg-gray-100">
                            <td class="w-1/6 p-2 text-center border-b">{{ i + 1 }}</td>
                            <td class="w-2/6 p-2 text-start border-b text-sm">{{ producto.nombre }}</td>
                            <td class="w-2/6 p-2 text-center border-b text-sm">{{ producto.categoriaId }}</td>
                            ...
                            <td class="w-2/6 p-2 text-center border-b text-sm">
                                <div class="flex justify-center space-x-3">
                                    <mat-icon class="text-amber-400 hover:text-amber-500 cursor-pointer" (click)="goEdit(producto.id)">edit</mat-icon>
                                    <mat-icon class="text-rose-500 hover:text-rose-600 cursor-pointer" (click)="goDelete(producto.id)">delete_sweep</mat-icon>
                                </div>
                            </td>
                        </tr>
                        </tbody>
